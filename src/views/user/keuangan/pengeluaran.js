...
const Pengeluaran = () => {
  ...
  const fetchData = async () => {
      try {
        const url = localStorage.getItem('role') === 'koordinator' ? 'https://smrapi.my.id/koordinatorKecakapan/' : 'https://smrapi.my.id/user/';
        ...
        const pengeluaran = await axios.get('https://smrapi.my.id/pengeluaran/'+response.data.kegiatan_id, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleAddPengeluaran = async (event) => {
    ...
    try {
      const response = await axios.post('https://smrapi.my.id/pengeluaran', formData, {
        headers: {
  ...
  const handleUpdatePengeluaran = (event) => {
    ...
    axios.post(`https://smrapi.my.id/updatePengeluaran/${editItem.id}`, updatedPengeluaran, {
      headers: {
        'Content-Type': 'multipart/form-data',
        Authorization: `Bearer ${localStorage.getItem('token')}`,
      },
  ...
  const handleDelete = (id) => {
    axios.delete(`https://smrapi.my.id/pengeluaran/${id}`, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${localStorage.getItem('token')}`,
      },
  ...
  return (
    <CCard className="p-4 shadow-sm" sm={6}>
      ...
      <CModal visible={modalNotaVisible} onClose={() => setModalNotaVisible(false)}>
        ...
        <CModalBody>
          {selectedNota ? <img src={`https://smrapi.my.id/nota/${selectedNota}`} alt="Nota" style={{ width: '100%' }} /> : <p>Nota tidak tersedia.</p>}
        </CModalBody>
      </CModal>
    </CCard>
  )
