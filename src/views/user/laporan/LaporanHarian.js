...
const LaporanHarian = () => {
  ...
  useEffect(() => {
    const fetchKegiatan = async () => {
      try {
        const response = await axios.get('http://localhost:8080/daftar_kegiatan/' + localStorage.getItem('kegiatan_id'), {
          headers: {
    ...
    const fetchLaporan = async () => {
      try {
        const response = await axios.get('http://localhost:8080/laporan_harian/' + localStorage.getItem('kegiatan_id'), {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleAddSubmit = async (event) => {
    try {
      ...
      const response = await axios.post('http://localhost:8080/laporan_harian', formData, {
        headers: {
  ...
  const handleUpdateSubmit = async (event) => {
    try {
      ...
      formData.append('tugas', selectedTugas.map(t => t.value));
      formData.append('deskripsi_laporan', event.target.description.value);
      const response = await axios.put('http://localhost:8080/laporan_harian/'+selectedDay.laporanHarian[0].id, formData, {
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
