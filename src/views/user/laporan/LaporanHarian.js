...
const LaporanHarian = () => {
  ...
  useEffect(() => {
    const fetchKegiatan = async () => {
      try {
        const response = await axios.get('https://smrapiii.000webhostapp.com/daftar_kegiatan/' + localStorage.getItem('kegiatan_id'), {
          headers: {
    ...
    const fetchTarget = async () => {
      try {
        const response = await axios.get('https://smrapiii.000webhostapp.com/target/' + localStorage.getItem('kegiatan_id'), {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
    ...
    const fetchLaporan = async () => {
      try {
        const response = await axios.get('https://smrapiii.000webhostapp.com/laporan_harian/' + localStorage.getItem('kegiatan_id'), {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleAddSubmit = async (event) => {
    try {
      ...
      const response = await axios.post('https://smrapiii.000webhostapp.com/laporan_harian', formData, {
        headers: {
  ...
  const handleUpdateSubmit = async (event) => {
    try {
      ...
      formData.append('tugas', selectedTugas.map(t => t.value));
      formData.append('deskripsi_laporan', event.target.description.value);
      const response = await axios.put('https://smrapiii.000webhostapp.com/laporan_harian/'+selectedDay.laporanHarian[0].id, formData, {
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
